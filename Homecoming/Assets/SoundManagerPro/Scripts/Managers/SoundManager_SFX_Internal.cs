...
#endif
                if (!ownedSFXObjects[i].GetComponent<AudioSource>().isPlaying)
			    {
		...
		for(int i = unOwnedSFXObjects.Count - 1; i >= 0; i--)
		{
			if(unOwnedSFXObjects[i] != null)
				if(unOwnedSFXObjects[i].GetComponent<AudioSource>() != null)
					if(unOwnedSFXObjects[i].GetComponent<AudioSource>().isPlaying || unOwnedSFXObjects[i].GetComponent<AudioSource>().mute)
						continue;
