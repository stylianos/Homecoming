...
public class vp_Grab_mine : vp_Interactable
{
	...
	protected virtual void StopGrab()
	{
		...
		if (Input.GetButton("Fire1"))
		{
			...
			if (m_Transform.GetComponent<Rigidbody>() != null)
			{
				m_Transform.GetComponent<Rigidbody>().AddForce(m_Controller.Velocity + m_Player.Forward.Get() * ThrowStrength, ForceMode.Impulse);
		...
		else	// otherwise perform a normal drop
		{
			...
			if (m_Transform.GetComponent<Rigidbody>() != null)
				m_Transform.GetComponent<Rigidbody>().AddForce(m_Controller.Velocity + m_Player.Forward.Get(), ForceMode.Impulse);
	...
	protected virtual void StartAlternativeInteract(){
		...
		m_InteractManager.m_CurrentInteractable = null;
