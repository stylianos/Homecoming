...
public class vp_FPWeaponMeleeAttack : vp_Component
{
	...
	protected override void Start()
	{
		...
		//Debug.Log ("Eimai prin to base " + splash +" kai to test einai " + test + "kai to test2 eiani " + test2);
		base.Start();
		//Debug.Log ("EIMAI STO START KAI TO SPLASH EINAI" + splash);
	...
	private void UpdateAttack()
	{
		...
		vp_Timer.In(SwingDelay, delegate()
		{
			...
			vp_Timer.In(ImpactTime, delegate()
			{
				...
				Debug.Log ("Edo ekana mia diafora kai to splash einai " + splash);
					
				if ( !test ) {
						Debug.Log("dn eimaia splash");
						...
						Debug.Log("eimai splash");
						DamageRadius = 4;
						DamageRange = 4;
						//RaycastHit[] hits = Physics.SphereCastAll(ray, DamageRadius, DamageRange, vp_Layer.Mask.BulletBlockers);
						// hit something: perform impact functionality
					
						Collider[] hits = Physics.OverlapSphere(this.transform.position , DamageRadius , vp_Layer.Mask.BulletBlockers);
						if (hits != null)
						{
							//Debug.Log("hits lenght is " + hits.Length);
							//this.GetComponentInChildren<ParticleSystem>().Play();
							foreach ( Collider target in hits ) {
								//Debug.Log("Target name" + target.transform.name);
								if (target.CompareTag("enemy")){
									SpawnImpactFX_Collider(target);

									ApplyDamage_collider(target);
	...
	void SpawnImpactFX_Collider(Collider hit)
	{
		...
		*/
		...
		{
			if (Random.value < SparkFactor)
				Object.Instantiate(m_SparkPrefab, m_Weapon.transform.position, m_Weapon.transform.rotation);
		}
	...
	void ApplyDamage_collider(Collider hit)
	{

		// do damage on the target
		hit.SendMessage(DamageMethodName, Damage, SendMessageOptions.DontRequireReceiver);

		// if hit object has physics, add the damage force to it
		Rigidbody body = hit.attachedRigidbody;
		if (body != null && !body.isKinematic)
			body.AddForceAtPosition((m_Camera.Transform.forward * DamageForce) / Time.timeScale, m_Controller.transform.forward);
