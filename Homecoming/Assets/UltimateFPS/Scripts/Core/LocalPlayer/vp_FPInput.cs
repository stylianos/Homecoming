...
public class vp_FPInput : MonoBehaviour
{
	...
	public states_handler states_script;
	...
	public void Start(){
		...
		states_script = (states_handler)player.GetComponent(typeof(states_handler));
	}
	...
	void InputContinuousInteract(){
		
		if(Input.GetKey(KeyCode.O)){
			GetComponent<vp_FPInteractManager>().is_o_pressed = true;
			if ( this.GetComponent<vp_FPInteractManager>().is_moving_platform == true)
				Player.ContinuousInteract.TryStart();
			
		}
		else{
			GetComponent<vp_FPInteractManager>().is_o_pressed = false;
			Player.ContinuousInteract.TryStop();
		}
			
	
	}
	
	void AlternativeInteract(){
		
		if(Input.GetKeyDown(KeyCode.G)){
			Player.AlternativeInteract.TryStart();
		}
		else
			Player.AlternativeInteract.TryStop();
	...
	/*
	protected virtual void InputAlternativeAttack()
	{
		...
		if (Input.GetMouseButton(1))
			Player.AlternativeAttack.TryStart();
		else
			Player.AlternativeAttack.TryStop();
