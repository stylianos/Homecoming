...
public class vp_FPInteractManager : MonoBehaviour
{
	...
	public Dictionary<Collider,vp_Interactable> m_Interactables = new Dictionary<Collider, vp_Interactable>(); // for testing interactable components
	...
	public bool is_moving_platform;
	public GameObject moving_platform;
	public bool is_o_pressed;
	...
	protected virtual bool CanStart_AlternativeInteract()
	{
		...
		if(FindInteractable( out interactable ))
		{
			...
			// check if we can interact with the interactable, if so, carry on
			//EDO TIN KALEI THN ALTERNATIVE
			if(!interactable.TryAlternativeInteract(m_Player))
				return false;
	...
	protected virtual bool CanStart_ContinuousInteract()
	{
		...
		if(FindInteractable( out interactable ))
		{
			...
			// check if we can interact with the interactable, if so, carry on
			//EDO TIN KALEI
			if(!interactable.TryContinuousInteract(m_Player)){
				return false;
			}
	...
	protected virtual bool CanStop_ContinuousInteract()
	{
		Debug.Log("Can stop continuos");
		return true;
	}
	
	protected virtual void OnStop_ContinuousInteract(){
		Debug.Log("On stop stop");
	...
	protected virtual bool ShouldFinishInteraction()
	{
		...
		//Debug.Log("To interactacle ienai " + m_Player.Interactable.Get());
		if(m_Player.Interactable.Get() != null)
		{
			Debug.Log("Mpikaaaaaaaa");
	...
	/// </summary>
	/// Den kserei ti eiani gia ayto stelnei minima
	protected virtual void OnControllerColliderHit(ControllerColliderHit hit)
	{	
		
		Rigidbody body = hit.collider.attachedRigidbody;
		if (is_o_pressed){
			if ( hit.gameObject.CompareTag("moving_platform") ) {
				hit.gameObject.GetComponent<Continuous_Interact>().TryContinuousInteract(m_Player);
				is_moving_platform = true;
				moving_platform = hit.gameObject;
			}
			...
			if ( !hit.gameObject.CompareTag("moving_platform") ) {
				...
				//ayto einai apla ama stamatisei na to pataei
					moving_platform.GetComponent<Continuous_Interact>().OnStop_ContinuousInteract();
					Transform[] temp = moving_platform.GetComponent<vp_MovingPlatform>().PathWaypoints.GetComponentsInChildren<Transform>();
					//the player can actually use it to summont the platform?
					moving_platform.transform.position = temp[1].position ; 
...
//Vasika einai to idio o enas pairnei to interactable kai kanei apeyhteies thn tryContinuious o allos to kanei me to hit collider kai 
//kalei thn TryInteract giati ayti pisteyei oti exie